...
async function processQueue() {
    if (uploadQueue.length === 0) { 
        isProcessing = false; 
        setTimeout(() => { document.getElementById('loading').style.display='none'; updateProgress(0); }, 800); 
        return; 
    }
    isProcessing = true;
    document.getElementById('loading').style.display = 'block';
    const file = uploadQueue.shift();
    document.getElementById('status-msg').innerText = "AI Menganalisis...";
    
    try {
        updateProgress(20);
        // Optimasi resolusi agar AI kilat
        const img = new Image();
        ...
        const fastBlob = await new Promise(r => canvas.toBlob(r, 'image/jpeg', 0.85));
        updateProgress(40);
        
        const blob = await imglyRemoveBackground(fastBlob);
        const noBg = await blobToBase64(blob);
        const original = await blobToBase64(fastBlob);
    ...
    photos.forEach((p, i) => {
        ...
        div.innerHTML = `
            <div style="display:flex; gap:12px; align-items:center;">
                <div style="width:70px; height:${boxH}px; border:2.5px solid var(--blue); border-radius:10px; overflow:hidden; background-image:url('${p.current}'); background-size:cover; background-position:center ${p.offset}%"></div>
                <div style="flex:1">
                    <input type="range" style="width:100%" min="0" max="100" value="${p.offset}" oninput="photos[${i}].offset=this.value; saveAll(); updatePreview(); renderPhotoList()">
                    <div style="display:flex; gap:6px; margin-top:8px">
                        ${!isBg ? `<input type="number" value="${p.qty}" onchange="photos[${i}].qty=parseInt(this.value); saveAll(); updatePreview()" style="width:40px; padding:5px; border-radius:6px; border:1px solid #ddd;">` : ''}
                        <button onclick="downloadManualCrop(${i})" style="flex:1; background:#059669; color:#fff; border:none; padding:10px; border-radius:8px; font-weight:800; cursor:pointer">💾 DOWNLOAD</button>
                        <button onclick="photos.splice(${i},1); saveAll(); renderPhotoList(); updatePreview();" style="background:#ef4444; color:#fff; border:none; padding:10px; border-radius:8px; cursor:pointer">🗑️</button>
                    </div>
                </div>
            </div>
